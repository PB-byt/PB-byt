...
def main():# Main function to run the app,Initialize session state for page navigation
    ...
    with tab_h:
        ...
        if st.session_state.page == 'home':
            ...
            # st.title('to') ⟫⁙※◈
            st.title("‖D'R JEWELS")
            st.write(f'<h4 style="color:pink;">◈ Hand-made with love. (✿◡‿◡) </h2>',unsafe_allow_html=True)
            st.write(f'<h5 style="color:pink;">◈   and fully customizable</h2>', unsafe_allow_html=True)
            ...
            a.markdown('<h3 style="color:pink;">starting at just *₹49*</h3>',unsafe_allow_html=True)
        ...
        # Product Detail Page
            ...
            with c:
                ...
                with cont:
                    st.write("price")
                    st.subheader(f'₹{current_pr['price']}')
    ...
    # order placing page-
            ...
            st.write("We will reach you within a few hours.🤗")
            st.divider()
            st.caption("copy this ↡")
            st.write(f"◌ | {current_pro['name']} |")
            st.write(f"▻ Product id : {current_pro['id']},")
            st.write(f'▻ Price: {current_pro['price']} ₹ only.')
            st.write(f"▻ Quantity : 1")
            st.divider()
            # ▷▻▣◊◌
            if st.button('<- back'):
                navigate('product_detail')
            st.caption("Happy Shopping 🤗 and have a good day.")
